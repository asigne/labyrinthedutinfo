...
public abstract class Case implements Serializable, Cloneable {
	int identifiant; // identifiant pour les cases poss�dante une image sinon 0
	...
	public Case sauvCase() {
		...
		if (caseASauv instanceof L) {
			sauvCase = new L(0, 0, 0); // creation d'une nouvelle case L si la
			// case a sauver est de type L
		} else if (caseASauv instanceof T) {
			sauvCase = new T(0, 0, 0); // creation d'une nouvelle case T si la
			// case a sauver est de type T
		} else {
			sauvCase = new I(0); // creation d'une nouvelle case I si la case a
			// sauver est de type I
		}
