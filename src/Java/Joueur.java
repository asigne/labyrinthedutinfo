...
public abstract class Joueur implements Serializable, Cloneable {
	...
	public void setPosition(int posLigne, int posColonne) {
		...
		partieActuelle.getMonPlateau().getCase(posLigne, posColonne)
		.ajouterJoueur(this);
	}
	...
	public boolean testCarteTrouvee() {
		Case maCase = partieActuelle.getMonPlateau().getCase(posLigne,
				posColonne);
		if (partieActuelle.getRegle().equals("Normal")) {
			if (this.getListCarte().size() > 0) {
				if (this.getCarteObjectif().getIdentifiant() == maCase
						.getIdentifiant()) {
			...
			if (this.getListCarte().size() > 0) {
				...
				while (i < this.getListCarte().size()) {
					if (this.getListCarte().get(i).getIdentifiant() == maCase
							.getIdentifiant()) {
						numObjectifTrouve = maCase.getIdentifiant();
						return true;
						// objectif=true;
				...
				/*
				 * if(objectif) { //carte trouvée !!! return true; }
				 */
			}
		}
		return false;
	...
	public void testJoueurGagnant() {
		if (posColonne == posColonneInit && posLigne == posLigneInit
				&& ListCarte.isEmpty()) {
			partieActuelle.setPartieFinie(true); // partie terminée
		}
	}

	// methode effectuant le test pour connaitre la cases accessibles par un
	// joueur
	...
	// fonction permettant de savoir si une case est accessible depuis la
	// postion du joueur
